...
- 由一条本地 Codex 自动化在七个固定节点执行并安全补跑必要节点。
...
## Legacy/fallback 恢复与 manager 运行确认

**以下是旧常驻 manager 的回退说明，不是 Codex 自动化的正常启动流程。执行 Codex 自动化切换或该自动化已启用时，不得启动、重启或安装 `run-manager.mjs`。**
...
仅在明确选择 legacy/fallback 回退时，修改代码后先运行完整验证，再由操作者在可见终端中重启真实服务：
...
- [ ] 在 `00:00` 执行前保留一个可识别的上一工作日状态，运行后确认生成的是上一工作日复盘，日期归属正确且重跑不重复。
- [ ] 对一个隔离的项目交付项提交可验收证据，完成自动或人工验收，确认项目 Markdown 受管区、项目进度和项目变更记录只写回一次；重跑后内容和进度不重复。
- [ ] 在 Codex 自动化页面创建一条本地执行的自动化，绑定“N哥时间管理”项目，时区选 `Asia/Shanghai`，每天调度设为 `00:00`、`08:00`、`09:00`、`12:00`、`15:00`、`18:00` 和 `21:00`，并将状态设为“已启用”。只创建这一条，不要建七条。自动化 prompt 必须原样使用：
...
- [ ] 确认自动化是本地执行、只有一条、绑定的项目正确，七个节点、`Asia/Shanghai` 和“已启用”均可见。切换“禁用”并确认不再调度，再切回“启用”并确认下次运行时间可见；不要通过删除记录来验证。
...
## Legacy/fallback：HTTP/手工 dispatch 快速开始

**这不是 Codex 自动化的正常运行方式。自动化切换期间不得用本节命令启动常驻服务。**
...
启动本地服务（legacy/fallback；Codex 自动化切换或已启用时不要执行）：
...
飞书任务清单（本机 `.env` 中的 `FEISHU_TASKLIST_GUID`）：
